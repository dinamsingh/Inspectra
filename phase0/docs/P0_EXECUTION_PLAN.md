...
### 1.6 Panel → measured-glyph assignment — **RESOLVED (STEP 7), B5 CLOSED**
...
**The assignment is now committed** in `fixtures/physical/coupons/glyph_selection.json`
(`selection_hash 1a1b1dabea1421f2…`), generated by a closed-form rule rather than chosen
panel by panel — `B5_GLYPH_SELECTION.md`:
...
It is a **complete 5 × 4 factorial with one panel per cell**: each of the 5 heights on
exactly 4 panels, each of the 4 shapes on 5, each of the 4 fonts on 5, all 20
(height, shape) pairs distinct, all 20 (font, height) pairs distinct, both shape classes
present, and P2's 3 mm class on **P03, P06, P09, P20** → 48 observations.  Every one of
those claims is asserted by a test, not stated in prose.

A weighted 8-3-3-3-3 alternative favouring P2 was considered and rejected: it would drop
the hardest height (1.2 mm) to 3 panels and would presume, before any data exists, where
the answer lies.  Stated limitation: P2's *between-panel* component rests on 4 panels — an
inherited property of the frozen 20-panel, one-glyph-per-panel design (A-14), which no
allocation removes.

**B3 does not decide this and did not.**  `glyph_map.json` covers all 400 deliberately so
this selection could be made without the map encoding a preference; every selected
`glyph_key` resolves in it, which `tools/validate_selection.py` checks.
...
| **B5** | **CLOSED (STEP 7).**  `fixtures/physical/coupons/glyph_selection.json` commits the assignment before any capture exists, generated by a closed-form rule (§1.6) rather than chosen panel by panel: a **complete 5 × 4 factorial with one panel per cell** — 4 panels per height, 5 per shape, 5 per font, all 20 (height, shape) and all 20 (font, height) pairs distinct, both shape classes, and P2's 3 mm class on P03/P06/P09/P20 = 48 observations.  Nine balance invariants are asserted by tests rather than by prose.  It also closes **C9**: the P1 cross-check subset **is** exactly the 20 measured glyphs — 20 ≥ 15 and spanning all heights, shapes and fonts — because P1 must bound the reference values P2/P3/P4 actually consume; leaving a measured glyph out is `MEASURED_GLYPH_NOT_CROSS_CHECKED`.  Enforcement: `--check` re-derivation, `validate_selection.py`, the manifest's `selection_hash`, and `run_real_batch.py` exiting on an unselected glyph.  49 tests | Choosing after capture is post-hoc selection; choosing badly leaves a criterion uncomputable | Done.  Residual, non-blocking and stated: P2's *between-panel* component rests on 4 panels — inherited from the frozen 20-panel, one-glyph-per-panel design (A-14), not from the allocation — and shapes clump slightly within a print sheet |
| **B6** | **SPECIFICATION RESOLVED (STEP 8) — still OPEN.**  `B6_EQUIPMENT_INVENTORY.md` completes this §13 inventory: **20 catalogued items, 14 mandatory**, each with its purpose, which blockers it serves, an access classification, and the evidence that would make it "available"; `tools/validate_inventory.py` gates it as `B6_INVENTORY_VERIFIED = YES\|NO`, where `UNKNOWN`, `ACCESS_NEEDED`, a placeholder, a missing calibration record where one is documented, a short quantity, or the same object listed twice all read as `NO`.  It deliberately **does not** duplicate the existing gates — the inventory records existence and paperwork, `verify_b2_setup.py` records reference-tier capability, `validate_flatness.py` records the B4 measurements — and cross-checks ids so one object cannot acquire two identities.  One classification is corrected: §13 files the **microscope under PREFERRED** while its own MISSING row says that without it P1 is uncomputable and no accuracy claim may be made, so B6 treats it as **MANDATORY**.  Explicitly rejected as unnecessary: CMM / optical comparator (the caliper survey already contributes ~0.0006 mm on a 3 mm glyph) and a dial indicator.  Packaged products are **samples, not equipment**, and recording one is refused.  42 tests | The protocol silently assumes all of it.  A missing item changes the design, not just the schedule | **Open: not one physical item has been confirmed**, so the gate returns `NO` — the correct answer today.  Three specialist-access items (scanner, certified standard, microscope), one shared caliper, six cheap consumables, two handsets, one tool ≈ two access visits.  Residual: the documents name no method for certifying a **backing plate's own flatness** |
...
**This inventory is now completed and machine-checkable in `B6_EQUIPMENT_INVENTORY.md`**
(STEP 8): 20 catalogued items, **14 mandatory**, each with purpose, the blockers it
serves, an access classification and the evidence required, gated by
`tools/validate_inventory.py` → `B6_INVENTORY_VERIFIED = YES | NO`.  The tables below are
retained as the source they were derived from.  Two corrections that document records:
the **measuring microscope is MANDATORY**, not PREFERRED (its own MISSING row says that
without it no accuracy claim may be made), and the **straightedge and gap gauge** that
B4 needs were never listed here at all.
...
measured.  **B5 is CLOSED** (STEP 7): the panel→measured-glyph assignment is committed as
a closed-form balanced factorial, and the P1 cross-check subset with it.  **B6's
specification is RESOLVED** (STEP 8) — the equipment inventory, its evidence rules and its
gate exist — but **B6 stays OPEN** because not one physical item has been confirmed.

**Every blocker's *specification* is now written and machine-checked.  What remains is
entirely physical:** reference values (B2), surveyed panels (B4) and confirmed equipment
(B6).  The overall verdict stays NO until those three are closed.
